...
if (isset($_GET["btnEliminar"])) {
    ...
    $consulta = "DELETE FROM `equiporegistro` WHERE (`UsuIdUsuario`='" . $_GET['id'] . "');";
    $conexion->query($consulta);
    if ($consulta) {
        echo "<script>if(alert('Registro Eliminado Correctamente')){
            document.location='../inicio.php';}
            else{ document.location='../inicio.php';
               }</script>";
    } else {
...
if (isset($_GET["btnFinalizarT"])) {
    ...
    $consulta = "Update `equiporegistro` set UsuEstadoSolic=3,UsuFechFin=NOW() WHERE (`UsuIdUsuario`='" . $_GET['id'] . "');";
    $conexion->query($consulta);
    if ($consulta) {
